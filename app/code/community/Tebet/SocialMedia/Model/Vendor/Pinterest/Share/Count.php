...
class Tebet_SocialMedia_Model_Vendor_Pinterest_Share_Count
{
    const API_RESOURCE_URL = 'http://api.pinterest.com/v1/urls/count.json';
    ...
    public function getUrlCount($url)
    {
        ...
        $count = 0;
        if ($response) {
            $result = json_decode(str_replace(array('receiveCount', '(', ')'), array('', '', ''), $response->getBody()), true);
